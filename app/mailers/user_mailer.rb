class UserMailer < ApplicationMailer
	...
	def weekly_notification(user)
		if user.universities.present?
			...
			@completed_but_incomplete_projects = Project.where( :university_id => @user.universities.first.id ).where( :status => 1 ).where("description = '' or description is null or expected_outcome = '' or expected_outcome is null or end_user = '' or end_user is null or bidding_contractor = '' or bidding_contractor is null or bidding_number is null or bidding_award is null or bidding_proceed is null or financial_budget is null or financial_contract_price is null or financial_actual_cost is null or timeline_actual_start is null or timeline_actual_end is null or timeline_target_start is null or timeline_target_end is null or timeline_duration is null or timeline_extension is null or completed_by is null")
